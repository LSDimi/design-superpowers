...
**Full manage operations require L2 at minimum** — a design system must exist. At L0/L1 the classic operations (publish cascade, adoption analytics, lint gates) don't apply, but **do not simply bounce the user** — run a limited health-check instead.

- **L0 (Greenfield):** No DS and no design language. Redirect is appropriate: "There's nothing to manage yet. Run `/creative` → `/map-design` to establish a design language, then `/ds-make` to create a DS." Offer to kick that off.
- **L1 (DESIGN.md exists) — Limited health-check mini-workflow:** Do not redirect blindly. Instead:
  1. **State scope honestly:** name which classic ops are **N/A at L1 and why** (publishing cascade — no library to publish; adoption analytics — nothing instrumented to measure; lint gates — no governance tier defined). Never fabricate analytics/adoption numbers that can't be measured yet.
  2. **Survey what exists:** DESIGN.md coverage/freshness, the Figma file (via the configured adapter if set — else note it's uninspected), any ad-hoc tokens/components.
  3. **Identify the top 3 gaps to L2** and an ordered path (typically: formalize tokens via `/ds-make` → establish a Figma library → add a governance tier). Size each with a light **S/M/L (+ day-range)** estimate for consistency.
  4. Route concrete build steps to `/ds-make`.
...
1. **Fetch the component**: If a Figma file key is available, retrieve the component structure and existing properties via the configured Figma adapter (see `${CLAUDE_PLUGIN_ROOT}/skills/shared/figma-adapter.md`). If not, work from user-provided description.
