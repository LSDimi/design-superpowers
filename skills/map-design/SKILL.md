...
- **L1 (DESIGN.md exists):** Enrich mode. Read the existing `DESIGN.md` first (this is a **router pre-step — do it before any Figma crawl**, so updates are diff-driven, not read-after-crawl), then extract from the new artifact and propose additions or corrections to each section. There is **no `.ds-context.md`, so the Figma adapter is always unset** — if the source is a Figma file or live URL, either pitch PluginOS (`/figma-setup`) or proceed from screenshots/DOM and state that extraction is not backed by a live variable read (per `${CLAUDE_PLUGIN_ROOT}/skills/shared/figma-adapter.md`).
- **L2 (DS exists):** Refresh mode. Extract a snapshot of the current DS state from the DS library (via the configured Figma adapter) to update `DESIGN.md` to reflect the current truth.
...
## Modes beyond first-time extraction

The four sub-agents above extract from **one** source. These modes compose them for the common non-greenfield asks. Announce the mode you're running.

### Reconcile mode (two sources — any level)
Triggers: "sync DESIGN.md with our tokens repo", "reconcile the Figma library against the code", "diff these two design languages", "which of these is the source of truth".
1. Extract from **both** sources (run Token Extractor / Component Cataloger on each). At L2+, one side is often live Figma variables (via the adapter), the other an external JSON / repo / second library.
2. Produce a **drift report**, bucketing every difference: *source-only* (in A not B), *target-only* (in B not A), *diverged* (same name, different value), *aligned*. **Never auto-pick a winner** — state which side each side's owner treats as canonical and propose reconciliations.
3. Recommend a **going-forward sync workflow** (who owns which layer, cadence, tooling). Output a merged DESIGN.md only on confirmation; preserve prose where unchanged; ask before removing anything that looks orphaned.

### Audit / Triage mode (L2 — refresh with decisions)
Triggers: "what changed since we wrote this", "clean up DESIGN.md against current state", "what should we consolidate/retire".
1. Snapshot current state via the adapter; diff against existing DESIGN.md.
2. For each delta, assign a **triage verdict**: *fold-in* (adopt into DESIGN.md), *retire* (deprecate — route to /ds-make Deprecation Planner), *leave-alone* (intentional variation). Size each **impact × effort** (light S/M/L).
3. Name **N worst and N healthy** areas explicitly so the report has contrast, not just a flat list.

### Drift Audit mode (L3 — enterprise health)
Triggers: "audit the enterprise DS for drift", "where is the library being violated".
1. Run the adapter drift sequence: `lint_detached` + `analyze_overrides` + `find_non_style_colors` + `lint_naming` (via `${CLAUDE_PLUGIN_ROOT}/skills/shared/figma-adapter.md`). Cross-reference known uSpec exceptions before flagging.
2. Group findings by owner/squad and by rule type; produce a **Top-N interventions** section (impact-ranked) rather than an exhaustive dump.
3. Distinguish **systemic** drift (recurring across squads → governance/tooling fix) from **local** one-offs.
...
1. **Obtain the artifact**:
   ...
   - Figma file or node: Fetch the document tree via the configured Figma adapter (see `${CLAUDE_PLUGIN_ROOT}/skills/shared/figma-adapter.md`), passing the file key and node ID parsed from the URL. Request specific pages or frames, not the full file.
   - Live URL: **do not stop at a screenshot.** A screenshot loses the authored design tokens. Where a browser/devtools tool is available, also read the live DOM and computed styles: `getComputedStyle` on representative elements, and the `:root` / stylesheet **CSS custom properties** (`--color-*`, `--space-*`, etc.) — these are the authored tokens. Distinguish **authored** CSS values (custom properties, source rules) from **computed** values where possible, and note any styles injected late by scripts that a static read may miss. Fall back to a screenshot only if no DOM access exists, and flag the degraded coverage. Always include a **coverage note** (what was sampled vs. not).
...
2. **Color extraction**:
   - From Figma: Read fill and stroke values from nodes via the configured Figma adapter (see `${CLAUDE_PLUGIN_ROOT}/skills/shared/figma-adapter.md`). Collect unique color values.
...
3. **Typography extraction**:
   - Identify all distinct font family + weight combinations in use. Check each family against the inventories in `${CLAUDE_PLUGIN_ROOT}/skills/shared/font-availability.md`; families that are neither installed nor Figma-available get annotated inline.
...
## Typography
**Families:** <primary font> + <secondary/body font> — annotate missing ones: `<font> (not installed — source: <where to get it>)`
