...
- **L0 (Greenfield):** Scaffold a new DS from scratch. A DESIGN.md gives the richest input, but its absence is **not a hard gate** — judge by the request:
  - **Specific, self-contained token/component request** (e.g. "generate an 11-stop scale from #FF6B35", "spec a Button"): proceed directly. Produce the artifact, then *offer* "/creative + /map-design to establish the broader design language" as a next step — an offer, not a prerequisite.
  - **Broad "build me a design system" with no direction:** recommend running /creative first to establish a visual direction, then /map-design to generate DESIGN.md, so the DS has a coherent source of truth. Offer to proceed anyway with stated assumptions if the user prefers.
- **L1 (DESIGN.md exists):** Scaffold a DS using DESIGN.md tokens as the source of truth. Token Architect reads DESIGN.md and formalizes it. There is **no `.ds-context.md`, so the Figma adapter is always unset** — any step that would "inspect actual Figma usage" (e.g. usage-based consolidation, audit) must first pitch PluginOS (`/figma-setup`) or explicitly proceed from DESIGN.md inference and label counts as inferred, not measured (per `${CLAUDE_PLUGIN_ROOT}/skills/shared/figma-adapter.md`).
