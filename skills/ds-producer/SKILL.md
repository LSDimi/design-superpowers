---
name: ds-producer
description: Use when creating, updating, auditing, documenting, versioning, or publishing DS components, foundations, icons, or patterns. Also use when managing Shortcut tickets for DS work or running governance checklists.
---
...
This skill is the L3 (Enterprise) specialization invoked by `/ds-make` when `.ds-context.md` declares `ds.maturity: enterprise` or `governance.tier: enterprise`. At L0-L2, /ds-make uses its universal sub-agents directly.
...
- Review or enforce the DS governance checklist
...
4. **Foundations:** Apply DS foundations, verify via DS lint (`{{governance.lint.tool}}`)
...
6. **Verify:** Run DS lint (`{{governance.lint.command}}`) across affected files to confirm clean removal
...
### DS Lint (Configured Lint Tool — `{{governance.lint.tool}}`)

**When to run:** After every structural change, before peer review, before publishing. Lint failures are **blocking**.

**Two modes:**
- **Variable Check (Linter):** Scans nodes for rule types, flags violations, offers one-click fixes
...
**One-time setup:** Run the lint tool's library sync script on each DS library file to pre-build the library key map (~5K tokens total, run once). This enables library name resolution in the Auditor without a Figma API token.

**Linting rules the Producer must satisfy:**

| Rule | What it checks | Common failures |
|------|---------------|-----------------|
| `fill` | Fill bound to DS variable (not style, not unbound, not foreign collection) | Unbound fills, using paint styles instead of variables |
| `stroke` | Stroke color bound to DS variable | Same as fill but for strokes |
| `strokeWidth` | Stroke weight bound to DS FLOAT variable | Hardcoded stroke widths |
| `radius` | Corner radius bound to DS variable (per-corner) | Hardcoded border radius |
| `spacing` | Auto-layout padding + itemSpacing bound to DS variables | Hardcoded spacing values |
| `textStyle` | Text style is from the foundations library (`{{figma.libraries[role=tokens]}}`) | Missing text style, or non-DS text style |
| `textComponent` | Text wrapped in DS typography component (not detached) | Bare text nodes, detached typography components |
| `effectStyle` | Effect style from the DS | Missing or non-DS effect style |
| `gridStyle` | Grid/layout style from the DS | Missing or non-DS grid style |
| `width` / `height` | Dimensions bound to DS variables (when not FILL/HUG) | Hardcoded fixed dimensions |
...
**Key detail:** The DS lint tool determines "from the DS" by matching variable collection keys and style keys cached from the foundations library file. It is NOT name-based — it's key-based after sync.

**Scope options:** File (all pages), Page (current page), Selection
**Fix capability:** Can auto-apply correct DS variable bindings via suggestion chips
...
**What it does:** Three-stage pipeline measuring DS adoption across all product files:
1. Fetches all team Figma files via REST API
2. Counts every component instance from tracked libraries
3. Outputs CSV reports (per-component, per-file, per-team)

**Tracked libraries:** Use `{{figma.libraries}}` keys from `.ds-context.md`. Track all DS libraries plus any non-DS libraries for comparison.
...
**Key metric:** DS instances vs. non-DS library instances per team = DS adoption rate
...
- Figma adapter: request only the nodes needed, not entire files.
