...
**Lead with weight + color, not size.** Most UI text can stay one comfortable size while weight and color do the hierarchy work — this reads more refined than a ladder of sizes. De-emphasize secondary content (softer color) rather than bolding everything primary. Don't let the HTML tag level dictate visual weight — an `<h3>` used as sidebar metadata should look like metadata. When aligning text of mixed sizes on one line, align by **baseline**, not center or top. Prefer a hand-picked scale over a purely mathematical ratio (math ratios generate steps you never use).
