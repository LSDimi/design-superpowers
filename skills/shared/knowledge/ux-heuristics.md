...
Cite these by name alongside Nielsen's 10 when a finding is about targeting, choice, or timing.

| Law | Statement | Design lever / when to cite |
|-----|-----------|------------------------------|
| **Fitts's Law** | Time to acquire a target ∝ distance ÷ target size. Bigger + closer = faster. | Primary CTAs get large hit areas near the pointer's likely path; screen edges/corners are "infinitely large" (pin destructive-safe actions there). Cite for small/crowded tap targets, far-flung primary actions, tiny close buttons. Pairs with the 44×44px touch-target floor in `accessibility.md`. |
| **Hick's Law** | Decision time ∝ log₂(number of choices). More options = slower. | Reduce/chunk choices; use progressive disclosure and sensible defaults. Cite for overloaded menus, too many primary actions per view, sprawling settings. |
| **Miller's Law** | People hold ~7±2 chunks in working memory. | Chunk long forms/lists; group nav. Cite for ungrouped 10+ item sets. |
| **Jakob's Law** | Users expect your product to work like the others they already know. | Prefer conventional patterns over novel ones without cause. Cite for reinvented standard controls. |
| **Doherty Threshold** | Keep system response < 400ms or perceived productivity drops. | Optimistic UI, skeletons, immediate feedback. Cite for actions with no status under load (pairs with Nielsen #1). |
