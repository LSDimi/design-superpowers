...
**Craft rules:**
- **Whitespace first** — start a layout over-spaced and remove space; adding it after the fact always undershoots. Generous space signals quality.
- **Proximity carries meaning** — tighten spacing *within* a group and increase it *between* groups; grouping should be legible from spacing alone, before color or borders.
- **Keep step sizes distinct** — adjacent spacing tokens should differ by at least ~25% so a choice between them is obvious (the t-shirt scale above mostly does this; the 40→48px step on the 8px base is a minor exception).
- **Dual unit is fine** — an 8px unit for layout/section rhythm with a 4px "mini unit" for inside-component padding is a deliberate, common pattern (e.g. IBM Carbon), not an inconsistency.
- **Layout variety** — across a long page, give each section a distinct layout family; avoid repeating one template (e.g. three-equal-cards, or endless image+text zigzag) down the page.
