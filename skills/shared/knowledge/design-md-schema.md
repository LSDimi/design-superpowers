...
| 10 | **Motion** *(optional)* | Duration/easing tokens, signature transitions, reduced-motion rule |
| 11 | **Voice & Tone** *(optional)* | Product voice, terminology (preferred vs. avoid), microcopy conventions |

> Sections 10–11 are optional but first-class: include them whenever the source has motion or copy signals (`/map-design` and `/design-review` both look for them). Omit only if the artifact genuinely has none, and say so rather than scattering the cues into Depth/Theme.
...
**Section 10 — Motion (optional).** Document motion as tokens, not adjectives. A duration scale (`fast` 100ms, `base` 200ms, `slow` 300ms), an easing set (standard/entrance/exit curves), and 2–4 signature transitions (what animates on route change, modal open, list reorder). Always state the **`prefers-reduced-motion` rule** (what degrades to no-animation). Prefer animating `transform`/`opacity` over layout properties.

```markdown
## Motion
**Durations:** fast 100ms · base 200ms · slow 300ms
**Easing:** standard cubic-bezier(0.2,0,0,1); entrance …; exit …
**Signature transitions:** <what animates, when>
**Reduced motion:** honor prefers-reduced-motion — replace movement with instant/opacity-only.
```

**Section 11 — Voice & Tone (optional).** Capture the product's writing voice and, critically, a **terminology table** (preferred term vs. term to avoid, e.g. "Customer" not "User") plus microcopy conventions (sentence case vs. title case, button verb style, error-message tone). `/design` and `/design-review` use this to write and grade copy in the house voice.

```markdown
## Voice & Tone
**Voice:** <3 adjectives + one line>
**Terminology:** | Use | Avoid | | Customer | User |
**Microcopy:** buttons = imperative verb; errors = plain + recovery step; casing = sentence case.
```

**Optional `Used` column (Sections 2 & 3).** When a DESIGN.md is generated from a real file, add an optional **`Used`** column to the Color and Typography tables (instance count per token) so reuse-vs-one-off and drift impact are captured natively — e.g. a token used 1× is a consolidation candidate; used 47× is load-bearing. Populate it from the Figma adapter (`extract_palette` / `list_variables` counts) when available; omit the column when counts aren't measurable.
