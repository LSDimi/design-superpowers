...
**One-sentence justification test (gate every animation):** you must be able to finish "this animation communicates ___" with feedback, continuity, orientation, or earned delight. If the sentence is "it looked cool," cut it.

**Productive vs. expressive motion:** split motion into two registers and don't mix them on the same surface. *Productive* — task feedback and micro-interactions, short and efficient (roughly the `micro`–`small` tokens). *Expressive* — significant moments (onboarding, celebratory, hero reveals), longer and more characterful (up to the `large` token, with `medium` as the transitional duration for standard modals/drawers/panels). Data-dense/task UIs lean productive; marketing/editorial surfaces can afford expressive.
...
## Layout Transitions (FLIP / Shared Elements)

Morphing between two layouts prevents visual jarring and keeps context during UI changes. The FLIP technique (First-Last-Invert-Play) animates size/position changes as composited transforms instead of layout properties.

- Use your framework's layout-animation primitive (Framer Motion `layout` prop, `layoutId` for shared-element morphs) rather than hand-computing coordinates.
- Shared-element transitions (`layoutId`) are the highest-leverage P2 continuity tool: a card that grows into a detail view reads as *the same object*, not a swap.
- Reserve morphs for elements with a real identity relationship. Morphing unrelated elements fabricates continuity that confuses more than it orients.

## Engineering Craft

Implementation nuances that separate smooth motion from janky motion. Apply when the deliverable includes working front-end motion.

| Rule | Why | How |
|------|-----|-----|
| **One update loop** | Competing rAF loops (smooth-scroll lib + animation lib) cause per-frame stutter | Drive everything from a single ticker — e.g. GSAP: `gsap.ticker.add((t) => lenis.raf(t * 1000))`; scroll position and animation state must update in the same frame |
| **Scroll-scrub on the compositor** | Scroll-driven narrative (pin + scrub) lags if it triggers layout | Scrub composited properties only; pin with transforms, not `position` changes |
| **Lazy-load heavy effect libs** | Confetti/particle libraries cost bundle weight for a moment that may never trigger | Dynamic-import on the success event (`await import`, `ssr: false` where relevant) |
| **Clean up on unmount** | Orphaned tweens leak memory and fire on dead nodes | Scope animations (`gsap.context()` / `useGSAP`) so unmount kills active tweens |
| **GPU promotion, sparingly** | Layer promotion (`will-change`, `force3D`) buys smoothness but each layer costs GPU memory | Promote only elements that animate frequently; remove promotion when idle |
