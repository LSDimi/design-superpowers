...
### Grids & data tables (WAI-ARIA `grid` pattern)

Interactive tables / data grids need 2-D keyboard navigation — Tab alone is not enough.

- **Roving tabindex across cells:** exactly one focusable cell (`tabindex="0"`) at a time; all others `-1`. Tab moves **into/out of** the grid as a single stop; arrows move **within**.
- **Arrow keys:** ←/→ move by cell, ↑/↓ move by row. `Home`/`End` = first/last cell in row; `Ctrl+Home`/`Ctrl+End` = first/last cell in grid. `PageUp`/`PageDown` for paged scroll on large grids.
- **Editable cells:** `Enter`/`F2` enters edit mode; `Esc` cancels and restores focus to the cell.
- **Roles:** `role="grid"` (or native `<table>`), `role="row"`, `role="columnheader"`/`role="rowheader"`, `role="gridcell"`. Use `role="treegrid"` when rows expand/collapse.
- **Virtualized grids:** set `aria-rowcount`/`aria-colcount` to the **total** (not the rendered slice) and `aria-rowindex` per row so SR users hear "row 40 of 5,000".
- Sortable headers expose `aria-sort`; selection uses `aria-selected`.
