...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `figma.adapter` | enum | no | unset | `pluginos` (default — auto-written after the install pitch verifies PluginOS is connected), `figma-mcp` (manual-override only — never auto-written), or unset (triggers the install pitch on next Figma action). See `${CLAUDE_PLUGIN_ROOT}/skills/shared/figma-adapter.md`. |
...
**Library roles:** `tokens`, `foundations`, `components`, `patterns`, `icons`, `documentation`, `checklist`, `demo`, or any custom string. Role determines which cascade stage a library belongs to and is matched against `governance.cascade` entries (case-insensitive substring match).

### `tokens` — Token System

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `tokens.collections` | array of strings | no | `["primitive", "semantic", "component"]` | Token collection names in your variable system. Must be an array of strings — do not use objects with `name`/`tier` keys. |
...
| `governance.cascade` | array of strings | no | `[]` | Ordered list of publish stages. Each stage must pass QA before the next publishes. Typical enterprise cascade: `["Foundations", "Components", "Patterns (L1)", "Squad Patterns (L2)", "Final files"]`. Note: the field is `cascade`, not `cascade_order`. |
...
| `code.component_libs` | array of strings | no | `[]` | npm package names for DS component libraries. Note: the field is `component_libs` (plural), not `component_lib`. |
| `code.token_import` | string | no | — | npm package or path for token imports. |

### `fonts` — Font Inventory (see `font-availability.md`)

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `fonts.inventory_path` | string | no | `"fonts-inventory.txt"` | Locally installed font families, one per line (generated; gitignore it). |
| `fonts.figma_inventory_path` | string | no | — | Figma-available families (generated via the adapter when relevant). |
| `fonts.last_scan` | string (ISO date) | no | — | Probes re-run when older than 30 days. |
...
See `${CLAUDE_PLUGIN_ROOT}/skills/shared/examples/enterprise-ds-context.example.md` for a fully populated enterprise-tier `.ds-context.md`.
